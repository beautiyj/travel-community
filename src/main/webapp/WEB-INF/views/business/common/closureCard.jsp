...
- isClosed : 현재 마감 상태 (Integer, 0/1, 필수)

--%>
<c:set var="closedFlag" value="${param.isClosed == 1}" />
<div class="business-card business-closure-card" data-place-id="${param.placeId}">
    <div class="business-closure-card__head">
        ...
        <button type="button" class="business-toggle${closedFlag ? ' is-on' : ''}" aria-pressed="${closedFlag}">
            <span class="business-toggle__thumb"></span>
        </button>
    </div>
    <div class="business-closure-banner${closedFlag ? ' business-closure-banner--closed' : ' business-closure-banner--open'}">
        <span class="business-status-dot business-status-dot--${closedFlag ? 'closed' : 'open'}"></span>
        <span class="business-closure-banner__text"><c:choose>
            <c:when test="${closedFlag}">현재 예약 마감 상태입니다. 신규 예약이 차단됩니다.</c:when>
