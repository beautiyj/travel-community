...
-- =========================================================
-- 숙박 기간 예약 컬럼  (2026-07-29 추가)
-- visit_date를 체크인으로 쓰고, 체크아웃을 이 컬럼에 담는다.
-- 숙박만 값이 있고 맛집/관광지는 NULL(당일 방문) — 기존 단일날짜 예약은 1박으로 간주된다.
-- ⚠️ 이 컬럼이 없으면 예약 조회·생성이 실패한다(매퍼가 check_out_date를 참조).
-- =========================================================
ALTER TABLE RESERVATION ADD COLUMN check_out_date DATE NULL;
