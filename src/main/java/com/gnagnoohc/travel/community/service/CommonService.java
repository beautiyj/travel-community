...
public class CommonService {
	...
	public Map<String, Object> searchPlaces(String keyword, int page) {
		...
		List<Map<String, Object>> rows = dao.searchPlaces(keyword, offset, PLACE_SEARCH_PAGE_SIZE + 1);
	...
	public Map<String, Object> searchConfirmedPlaces(int memberId, String keyword, int page) {
		int offset = page * PLACE_SEARCH_PAGE_SIZE;
		List<Map<String, Object>> rows = dao.searchConfirmedPlaces(memberId, keyword, offset, PLACE_SEARCH_PAGE_SIZE + 1);
		return toPageResult(rows);
	}

	// PAGE_SIZE+1건 조회 결과를 {"items": 실제 PAGE_SIZE건, "hasMore": 다음 페이지 존재 여부}로 가공
	private Map<String, Object> toPageResult(List<Map<String, Object>> rows) {
		boolean hasMore = rows.size() > PLACE_SEARCH_PAGE_SIZE;
		List<Map<String, Object>> items = hasMore ? rows.subList(0, PLACE_SEARCH_PAGE_SIZE) : rows;
