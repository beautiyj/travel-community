...
<html lang="ko">
    ...
    <div style="margin-bottom: 24px;">
        ...
        <form action="${pageContext.request.contextPath}/tour/list" method="get">
            ...
            <!-- 지역 필터 선택 상태 유지용 hidden 필드 -->
            <c:if test="${not empty selectedRegionId}">
                <input type="hidden" name="regionId" value="${selectedRegionId}" />
            </c:if>
        </form>
        <!-- 지역 필터 버튼 클릭 시 기존 placeType 및 keyword 파라미터가 유실되지 않도록 변수 조합 -->
        <c:set var="typeParam" value="${not empty selectedPlaceType ? '&placeType='.concat(selectedPlaceType) : ''}" />
        <c:set var="kwParam"   value="${not empty keyword ? '&keyword='.concat(keyword) : ''}" />
        <!-- 지역 필터 버튼 바 -->
