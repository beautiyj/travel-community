-- =========================================================
-- Travel Community 전체 스키마 (DROP → CREATE 갈아엎기용)
--
-- ⚠️ 이 스크립트는 아래 모든 테이블을 DROP 후 재생성한다.
--    기존 데이터가 전부 삭제되며 되돌릴 수 없다. 로컬 개발 DB에서만 실행할 것.
--
...
-- =========================================================
-- 기존 테이블 제거 (FOREIGN_KEY_CHECKS=0 이므로 순서 무관하지만 자식→부모 순 정렬)
-- =========================================================
...
DROP TABLE IF EXISTS member_social_auth;
DROP TABLE IF EXISTS member_local_auth;
...
CREATE TABLE RESERVATION (
    ...
    order_id             VARCHAR(100) NULL,                  -- 결제 준비 시 발급한 주문번호 (정합성 보정 배치의 PG 조회 키)
    payment_type         TINYINT      NULL,                  -- 시도 중인 PG (1 토스 / 2 카카오), NULL=미시도
    pg_tid               VARCHAR(60)  NULL,                  -- 카카오 tid (카카오 주문조회는 tid 필요). 토스는 order_id로 조회하므로 NULL
    created_at           DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
ALTER TABLE PAYMENT ADD CONSTRAINT UQ_PAYMENT_RESERVATION UNIQUE (reservation_id);   -- 한 예약당 결제 1건 (이중결제 물리 차단)
