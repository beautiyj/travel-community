...
public interface CommonMapper {
	...
	List<Map<String, Object>> searchPlaces(@Param("keyword") String keyword, @Param("offset") int offset, @Param("fetchSize") int fetchSize);

	// 장소 태그: 방문자인증후기용 - 로그인 회원이 확정(결제완료)한 예약의 장소만 이름으로 검색
	// TODO: 마이페이지 결제완료 로직 완성 전까지 status='PAID' 기준으로 임시 구현. 완성되면 교체 필요.
	List<Map<String, Object>> searchConfirmedPlaces(@Param("memberId") int memberId, @Param("keyword") String keyword, @Param("offset") int offset, @Param("fetchSize") int fetchSize);
